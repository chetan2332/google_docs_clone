...
class HomeScreen extends ConsumerWidget {
  ...
  void navigateToDocument(String documentId, BuildContext context) {
        ...
        builder: (context, snapshot) {
          ...
          return Center(
            child: ListView.builder(
              itemCount: (snapshot.data!.data as List<DocumentModel>).length,
              itemBuilder: (context, index) {
                DocumentModel document = snapshot.data!.data[index];
                return InkWell(
                  onTap: () => navigateToDocument(document.id, context),
                  child: SizedBox(
                    height: 50,
                    child: Card(
                      child: Center(
